...
export default function Shop() {
  ...
  return (
    <>
      ...
      <section className={`transition-colors duration-700 ${mode === 'subscription' ? 'bg-near-black' : 'bg-warm-white'}`}>
        ...
        {mode === 'subscription' && (
          <AnimatedSection direction="scale">
            <div className="grid lg:grid-cols-[1.3fr_1fr] min-h-[85vh]">
                ...
                <div className="grid grid-cols-2 gap-2 mb-8">
                  {[
                    ...
                    <div key={b} className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-accent text-white/35">{b}</span>
                    </div>
                  ))}
                </div>
                  ...
                  <AnimatedSection key={product.id} delay={i * 0.1} direction="scale">
                    <div className="relative overflow-hidden border h-full flex flex-col transition-all duration-300 hover:-translate-y-1 bg-white border-near-black/[0.06] hover:shadow-xl">
                      ...
                      <div className="px-8 pb-8 flex flex-col flex-1">
                        <h3 className="text-3xl font-heading mb-1 text-near-black">{t(product.nameKey)}</h3>
                        <p className="text-sm font-accent leading-relaxed mb-6 flex-1 text-near-black/45">{copy.pitch}</p>
                        <div className="flex items-end justify-between mb-4">
                          <div>
                            <div className="flex items-baseline gap-2">
                              <span className="text-3xl font-heading text-near-black">{formatPrice(product.price[currency], currency)}</span>
                              {product.originalPrice && (
                                <span className="text-sm line-through text-near-black/25">{formatPrice(product.originalPrice[currency], currency)}</span>
                              )}
                            </div>
